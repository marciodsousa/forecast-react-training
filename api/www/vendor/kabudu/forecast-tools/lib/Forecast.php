...
namespace ForecastTools;

/**
 * Forecast.php
 */
use ForecastTools\Cache\CacheHandlerInterface;
use ForecastTools\Exception\ForecastException;
...
class Forecast
{
    ...
    public function getData($args1)
    {
        ...
        */
        $requests = null; // will hold array of arrays of lat/long/time/options
        ...
        // validate that the cache handler adheres to the interface
        if (isset($this->_cache_handler)) {
            if (!$this->_cache_handler instanceof CacheHandlerInterface) {
                throw new \InvalidArgumentException('Cache handler must implement interface: \ForecastTools\Cache\CacheHandlerInterface');
            }
        }
