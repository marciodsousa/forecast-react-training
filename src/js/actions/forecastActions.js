...
export function fetchForecast(input) {
    ...
    return function(dispatch) {
        ...
        axios.get("http://localhost:8080/public/index.php/forecastlocation/" + input.location.replace(" ", "%20"))
            .then((response) => {
                dispatch({ type: "FETCH_FORECAST_FULFILLED", payload: response.data })

                dispatch({ type: "FETCH_30DAY_FORECAST", payload: null })
                axios.get("http://localhost:8080/public/index.php/forecastlocation/" + input.location.replace(" ", "%20") + "/pastdays/30")
                    .then((response) => {
